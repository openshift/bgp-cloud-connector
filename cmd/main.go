...
func main() {
	...
	// A client that reads straight from the API server, for the startup lookups
	// that run before the manager's cache exists.
	...
	// Reads the trusted CA bundle's startup baseline straight from the API
	// server; the manager's cache is not running yet.
