...
// A request carrying one permission where the operator asks for six,
// which is the shape a release that changed the permission list leaves
// behind on a cluster installed by an earlier one.
func driftedRequest() *unstructured.Unstructured {
...
func TestResolveCredentials_SecretMissingClientIDIsAnError(t *testing.T) {
	...
	if !errors.As(err, &credErr) {
		t.Errorf("got %T, want a *platform.CredentialError: a secret the operator cannot use is a credentials problem, not a discovery one", err)
	}
}
...
// The request is kept up to date on a cluster that already has its
// secret, which is every cluster past the first reconcile. Taking the
// credential from the secret and returning there would mean a
// permission list changed in a later release never reached an install
// made by an earlier one, and nothing in the system would ever repair
// it.
func TestResolveCredentials_UpdatesADriftedRequestWithTheSecretInPlace(t *testing.T) {
	withValidation(t, nil)
	...
	if cred == nil {
		t.Fatal("ResolveCredentials returned no credential")
	}
